...
class Roundabout:
    ...
    def __init__(self):
        ...
        for i in range(8):
            self.segments[i].next_segment = self.segments[(i + 1) % 8]
    ...
    def run_sim(self, cars, start_time, sim_time):
        ...
        def check_rondo_state(sys_time):
            ...
            while state_change == True:
                ...
                for seg in self.segments:  # przeglądam wszystkie segmenty
        ...
        def force(sys_time):
            for i in range(0, 4):
                if len(self.queues[i]) == 0:
                    continue
                if self.queues[i][0].time_to_force <= sys_time:  # próba wymuszenia
                    if self.segments[segment_index(i)].o_car is None:
                        ...
                        self.segments[segment_index(i)].occupy(
                            car=self.queues[i][0],
                            time=sys_time + self.queues[i][0].segment_drive_time,
                        )
                        self.segments[segment_index(i)].next_segment.occupied = True
                        self.queues[i].pop(0)  # usuwam już autko z kolejki
                        # ---- tutaj następne autko musi zyskac czas na wymuszenie
                        add_time_to_force(cars=self.queues[i], sys_time=sys_time)
                        # print("Nastąpiło wymuszenie")
                        continue

        def moving_car_from_queue_to_rnd(i):
            added_time = 0
            if not self.queues[i][0].is_moving:
                added_time = self.queues[i][0].starting_drive_time
                self.queues[i][0].is_moving = True
            self.segments[segment_index(i)].occupy(
                car=self.queues[i][0],
                time=sys_time + self.queues[i][0].segment_drive_time + added_time,
            )
            self.segments[segment_index(i)].next_segment.occupied = True
            self.queues[i].pop(0)  # usuwam już autko z kolejki
        ...
        def check_entry_state(sys_time):
            ...
            for i in range(0, 4):
                ...
                if self.segments[segment_index(i)].occupied is False:
                    moving_car_from_queue_to_rnd(i)
                    add_time_to_force(cars=self.queues[i], sys_time=sys_time)
