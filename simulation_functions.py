...
def simulations_from_file(parameters, generate_new_file=False, filename="parameters.json"):
...
def calculate_sim_time(cars_out_list):
    ...
    for i in range(0, len(cars_out_list[0])):
        times_in_sim.append(
            cars_out_list[1][i] - cars_out_list[0][i].arrival_time)
...
def sim_call(parameters, summary=False):
    ...
    sim_time, warm_up_time, traffic_intensity, segment_drive_time_distribution, percentage_cars_on_main_road, force_intensity, starting_drive_time_distribution = parameters
